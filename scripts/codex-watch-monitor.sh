...
STATUS_FILE="$LOG_DIR/codex-watch-monitor.json"
STATE_FILE="$LOG_DIR/.codex-watch-monitor.state"
ACTIVITY_FILE="$LOG_DIR/.codex-watch-monitor.activity"
LATEST_LOG="$(ls -1t "$LOG_DIR"/codex-watch-[0-9][0-9][0-9][0-9]-*.log 2>/dev/null | head -n 1 || true)"
STALE_MINUTES="${CODEX_MONITOR_STALE_MINUTES:-20}"
...
if [[ "${1:-check}" == "status" ]]; then
  if [[ -f "$STATUS_FILE" ]]; then
    ...
    echo '{"state":"unknown","detail":"no codex monitor status yet","log_file":"","inactive_seconds":null}'
  fi
  exit 0
fi
...
capture=""
pane_command=""
if capture="$("$ROOT_DIR/scripts/codex-watch.sh" capture 2>/dev/null)"; then
  session_running=true
  pane_command="$("$ROOT_DIR/scripts/codex-watch.sh" command 2>/dev/null || true)"
fi

state="idle"
detail="No running codex-watch session"
inactive_seconds=""
...
completion_regex='(^READY_FOR_NEXT_TASK:|^Closeout:|^Done:)'
...
if [[ -n "$unsafe_runtime_process" ]]; then
  ...
  if [[ "$pane_command" =~ ^(zsh|bash|sh|fish)$ ]]; then
    state="shell"
    if printf '%s\n' "$capture_tail" | grep -qi "conversation interrupted"; then
      detail="codex-watch session is active but Codex returned to shell after an interruption"
    else
      detail="codex-watch session is active but Codex is not running in the pane"
    fi
  elif printf '%s\n' "$capture_tail" | grep -E -q "$completion_regex"; then
    state="ready"
    detail="Ready-for-next-task markers detected in codex-watch output"
  elif [[ -n "$inactive_seconds" && "$inactive_seconds" -ge "$STALE_SECONDS" ]]; then
    state="stalled"
    detail="No codex-watch pane changes for ${STALE_MINUTES}+ minutes"
  else
    state="running"
    detail="codex-watch session is active"
  fi
fi
...
if [[ "$current_fingerprint" != "$previous_fingerprint" ]]; then
  ...
  case "$state" in
    ready)
      notify "codex-watch ready" "$detail"
      ;;
    stalled)
      notify "codex-watch may be stuck" "$detail"
      ;;
    unsafe_runtime)
      notify "codex-watch unsafe runtime" "$detail"
      ;;
  esac
fi
